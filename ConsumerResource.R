...
num_phase + tra_phase + tra_deltap0 + num_early0 + tra_early0 + guide_area() + plot_layout(guides = "collect")

##### Extras ##### 

# Won't appear in the final buildup

# Examine final population at \Delta p=0

num_deltap0 = 
  num_results %>% 
  filter(deltap == 0) %>% 
  pivot_longer(cols = 2:3, names_to = "Species", values_to = "Population") %>% 
  ggplot(aes(x = early, y = Population, color = Species)) +
  geom_point() + 
  geom_line() + 
  scale_color_manual(values = color_scheme, labels = c("1", "2")) + 
  ylab("log10(Final Population)") +
  scale_x_continuous(name = "Generations early", breaks = c(0, -40, -80), label = c(0, -40, -80)) +
  theme_bw() + 
  # ggtitle("C") + 
  coord_flip() + 
  scale_y_log10() +
...
# Examine final population when the two species arrive at the same generation

num_early0 = 
  num_results %>% 
  filter(early == 0) %>% 
  pivot_longer(cols = 2:3, names_to = "Species", values_to = "Population") %>% 
  ggplot(aes(x = deltap, y = Population, color = Species)) +
  geom_point() + 
  geom_line() + 
  scale_y_log10(name = "log10(Final Population)") + 
  scale_color_manual(values = color_scheme, labels = c("1", "2")) + 
  scale_x_continuous(name = expression(paste(Delta, p)), breaks = deltap_lst, label = deltap_lst) +
  theme_bw() + 
  ggtitle("D") +
  theme(axis.text = element_text(size = 15), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20),
        legend.text = element_text(size = 17), legend.title = element_text(size = 20), 
...
tra_deltap0 = 
  ...
  geom_point() + 
  geom_line() + 
  scale_color_manual(values = color_scheme, labels = c("1", "2"), guide = "none") + 
  scale_y_log10(name = "log10(Final Population)", breaks = c(1e-15, 1e-10, 1e-5, 1)) +
  scale_x_continuous(name = "Generations early", breaks = c(0, -40, -80), label = c(0, -40, -80)) +
  theme_bw() + 
  ggtitle("C") + 
  coord_flip() + 
...
tra_early0 = 
  ...
  geom_point() + 
  geom_line() + 
  scale_y_log10(name = "log10(Final Population)", breaks = c(1e-20, 1e-15, 1e-10, 1e-5, 1)) + 
  scale_color_manual(values = color_scheme, labels = c("1", "2"), guide = "none") + 
  scale_x_continuous(name = expression(paste(Delta, p)), breaks = deltap_lst, label = deltap_lst) +
  theme_bw() + 
  ggtitle("E") + 
